...
- **Web Dashboard**
  GitHub OAuth 로그인 기반 관리 UI. 연동 저장소, PR 리뷰 결과, 스킬 설정을 브라우저에서 확인·관리
...
- `GITHUB_CLIENT_ID` / `GITHUB_CLIENT_SECRET`: GitHub App의 OAuth 클라이언트 정보 (App Settings에서 확인)
- `ALLOWED_GITHUB_USERS`: 대시보드 접근을 허용할 GitHub 로그인명 (콤마 구분, 예: `okdoittttt`)
...
프론트엔드 개발 서버를 별도로 실행하려면:

```bash
cd frontend && npm run dev  # http://localhost:5173
```

> 자세한 프론트엔드 개발 환경 설정은 [`frontend/README.md`](frontend/README.md)를 참고하세요.
...
- **401 Unauthorized (Webhook)**: GitHub App ID나 Private Key 경로가 올바른지 확인하세요.
- **Invalid Webhook Signature**: `GITHUB_WEBHOOK_SECRET`이 GitHub 설정과 일치하는지 확인하세요.
- **LLM API Errors**: API Key가 유효한지, 할당량이 남아있는지 확인하세요.
- **로그인 후 403**: `ALLOWED_GITHUB_USERS`에 본인의 GitHub 로그인명이 포함되어 있는지 확인하세요.
- **로그인 콜백 오류**: GitHub App Settings의 **Callback URL**에 `http://localhost:8000/api/auth/callback`이 등록되어 있는지 확인하세요.
...
- [x] **Web Dashboard**: GitHub OAuth 로그인 기반 관리 UI 구현. 리뷰 결과 조회, 저장소 관리, 스킬 CRUD를 브라우저에서 제공
