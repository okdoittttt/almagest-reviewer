...
JSON만 응답해주세요."""
    ...
    # Diff 길이 제한
    patch_preview = file.patch[:10000] if len(file.patch) > 10000 else file.patch
    is_truncated = len(file.patch) > 10000
...
## Diff
{'(처음 10000자만 표시, 전체 길이: ' + str(len(file.patch)) + '자)' if is_truncated else ''}
